...
int main(int argc, char** argv)
{
	...
	timer_countdown.set_one_shoot(false);
	...
	while (true)
	{
		...
		else
		{
			...
			{
				stop_audio(_T("bgm"));
				MessageBox(hwnd, _T("输光了！"), _T("游戏结束"), MB_OK | MB_ICONINFORMATION);
				exit(0);
			}

			player_1.set_target(path.get_postion_at_progress((float)progress_1 / num_total_char));
			player_2.set_target(path.get_postion_at_progress((float)progress_2 / num_total_char));
		...
		else
		{
			...
			if (stage == Stage::Racing)
			{
				...
				{
					0,720 - img_ui_textbox.getheight(),
					img_ui_textbox.getwidth(),img_ui_textbox.getheight()
				};
