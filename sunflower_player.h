...
public:
	...
	void on_attack_ex()
	{
		...
		const Vector2D& bullet_size = bullet->get_size();
		const Vector2D& target_size = target_player->get_size();
		const Vector2D& target_position = target_player->get_position();
		...
		mciSendString(_T("play sun_text_ from 0"), NULL, 0, NULL);
...
private:
	...
	const Vector2D velocity_sun = { 0.25f,-0.5f };
};
