...
Barb::Barb()
{
	...
	animation_loose.add_frame(ResourcesManager::instance()->find_altas("barb_loose"));
	...
	animation_break.add_frame(ResourcesManager::instance()->find_altas("barb_break"));
	...
	timer_aim.set_one_shoot(true);
	timer_aim.set_on_timeout([&]()
		{
			if (stage == Stage::Aim)
			{
				stage = Stage::Dash;
				const Vector2D& pos_player = CharacterManager::instance()->get_player()->get_position();
...
void Barb::on_update(float delta)
{
	...
	switch (stage)
	{
	case Barb::Stage::Idle:	// 竖直方向的周期性变化
